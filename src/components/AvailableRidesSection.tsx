...
export const AvailableRidesSection: React.FC<AvailableRidesSectionProps> = ({
  ...
  const [maxDistanceFilter, setMaxDistanceFilter] = useState<number>(10); // km
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-4">
        ...
        {/* If BH or GH is selected in destination filter, show Hostel Number Selector 1 to 13 */}
        {isDestHostel && (
          <HostelSelector
            type={destHostelType}
        ...
        {/* Small detail below destination booking option */}
        <CampusRidePolicyDetail />

        {/* Secondary filters row: Distance slider, Location category, Vehicle, Girls-only */}
        <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4 text-xs">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
              <span className="text-slate-500 font-medium">Pickup Zone:</span>
              <select
              ...
              >
                ...
                <option value="Campus Gates">Campus Gates (Main / Law)</option>
                <option value="Hostels">Hostels (BH / GH)</option>
                <option value="Campus Hubs">Campus Hubs (UniMall/Library)</option>
      ...
      {filteredRides.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center max-w-xl mx-auto space-y-4">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            Try loosening your seat or destination filters, or be the first student to create a ride heading this way!
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRides.map((ride) => {
            return (
              ...
              >
                {/* Top Badge: Vehicle and Seats Open (NO PRICE SHOWN ON DRIVER CARD) */}
                <div className="flex items-center justify-between gap-2 mb-4">
                  ...
                  {/* Clean status badge instead of driver price */}
                ...
                {/* Driver Details (No Profile Picture) */}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5 truncate">
                        ...
                        {ride.driver.verifiedStudent && (
                          <span className="w-3.5 h-3.5 bg-emerald-500 rounded-full flex items-center justify-center text-white text-[9px] font-bold shrink-0" title="Verified Campus Driver">
                            ✓
                          </span>
                        )}
                      </div>
                    ...
                    <p className="text-[11px] text-slate-500 truncate">
                      {ride.vehicleType} • {ride.vehicleNumber || 'Campus Auto'} • {ride.driver.course}
                    </p>
                  </div>
                </div>
                ...
                {/* Live Location & Distance from user */}
                <div className="bg-slate-50 rounded-xl p-2.5 text-xs mb-4 space-y-1.5 border border-slate-100">
                  <div className="flex items-center justify-between text-slate-600">
                    <span className="text-[11px] text-slate-500 font-medium">Live Location:</span>
                    <span className="font-bold text-slate-800 text-[11px] truncate max-w-[170px] text-right">
                      {ride.currentLocationName}
                    </span>
                  </div>

                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-slate-500 font-medium">Distance from you:</span>
                    <span className="font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md text-[11px] border border-emerald-100">
                      {ride.distanceFromUserKm} km away
                    </span>
                  </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center gap-1.5">
                    {Array.from({ length: ride.totalSeats }).map((_, idx) => {
                      const isOccupied = idx < ride.occupiedSeats;
                      return (
                        <div
                          key={idx}
                          className={`flex-1 h-2 rounded-full transition-all ${
                            isOccupied ? 'bg-slate-300' : 'bg-emerald-500'
                          }`}
                          title={isOccupied ? 'Occupied seat' : 'Available seat'}
                        />
                      );
                    })}
                  </div>
                  <div className="flex justify-between text-[10px] text-slate-400 mt-1">
                    <span>{ride.occupiedSeats} Occupied</span>
                    <span className="text-emerald-600 font-semibold">{ride.availableSeats} Available</span>
                  </div>
                </div>

                {/* Optional Note */}
                {ride.notes && (
                  <p className="text-[11px] text-slate-500 italic bg-amber-50/50 p-2 rounded-lg border border-amber-100/50 mb-4 line-clamp-2">
                    "{ride.notes}"
                  </p>
                )}
