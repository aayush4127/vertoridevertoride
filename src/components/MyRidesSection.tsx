...
import { MyBooking, Ride, PageTab } from '../types';
import { 
  CalendarCheck, 
  MapPin, 
  Navigation, 
...
export const MyRidesSection: React.FC<MyRidesSectionProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Currently Active Ride Highlight (if any) */}
      {activeBookings.length > 0 && activeTab !== 'completed' && activeTab !== 'cancelled' && (
        <div className="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl relative overflow-hidden border border-indigo-800">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-white/10">
            ...
            {/* Boarding OTP Badge */}
            <div className="bg-white/10 border border-white/20 px-4 py-2.5 rounded-2xl flex items-center gap-3 self-start md:self-auto backdrop-blur-sm">
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-300 block">Boarding OTP</span>
                <span className="text-xl font-mono font-extrabold text-emerald-400 tracking-widest">
                  {activeBookings[0].boardingOtp}
                </span>
              </div>
              <ShieldCheck className="w-6 h-6 text-emerald-400 opacity-90" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-white/5 rounded-2xl p-4 border border-white/10 space-y-3">
              ...
              {/* Driver & Vehicle (No Profile Picture) */}
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-indigo-200 font-mono mt-0.5">
                    {activeBookings[0].ride.vehicleType} • {activeBookings[0].ride.vehicleNumber || 'Campus Auto'}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white/5 rounded-2xl p-4 border border-white/10 space-y-3">
              ...
              <div className="text-[11px] text-slate-300 pt-1 border-t border-white/10">
                <span className="text-emerald-300 font-bold">You saved ~₹90</span> compared to a solo auto trip to this destination!
              </div>
            </div>
          ...
          <div className="pt-4 border-t border-white/10 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => alert(`Simulating phone call to driver ${activeBookings[0].ride.driver.name} at ${activeBookings[0].ride.driver.phone}`)}
                className="px-4 py-2.5 rounded-xl text-xs font-bold bg-white/10 hover:bg-white/20 text-white border border-white/20 transition-all cursor-pointer flex items-center gap-2"
              >
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-4 py-2 rounded-xl text-xs font-bold bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/40 transition-all cursor-pointer flex items-center gap-1.5"
              >
        ...
        {displayedBookings.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-10 text-center max-w-md mx-auto space-y-3">
            ...
            <p className="text-xs text-slate-500">
              Ready to travel around LPU, Phagwara, or Jalandhar?
            </p>
            <button
              onClick={() => onNavigate('book')}
            ...
            {displayedBookings.map((b) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Driver summary (No Profile Picture) */}
                  <div className="flex items-center justify-between text-xs text-slate-500 pt-2 border-t border-slate-100">
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 rounded-md bg-slate-100 flex items-center justify-center text-xs">
                        {b.ride.vehicleType === 'E-Rickshaw' ? '🛺' : '🚕'}
                      </span>
                  ...
                  <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2 text-xs">
                    ...
                    {isActive && (
                      <button
                        type="button"
                        onClick={() => onTrackRide(b.ride)}
                        className="px-3 py-1.5 rounded-lg font-bold bg-indigo-600 text-white text-[11px] hover:bg-indigo-700"
                      >
                        Track Ride
                      </button>
                    )}
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in-95">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl space-y-2.5 text-xs text-slate-600 border border-slate-100">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-slate-800">{selectedReceiptBooking.bookedAt}</span>
              </div>
              <div className="flex justify-between">
                <span>Status:</span>
                <span className="font-bold uppercase text-indigo-600">{selectedReceiptBooking.status}</span>
              </div>
              <div className="flex justify-between">
            ...
            <div className="space-y-2 text-xs">
              <div className="font-bold text-slate-800">Route & Co-Passenger Details</div>
              <div className="bg-white p-3 rounded-xl border border-slate-200 space-y-1.5">
                ...
                <p><strong>Driver:</strong> {selectedReceiptBooking.ride.driver.name} ({selectedReceiptBooking.ride.driver.course})</p>
                <p><strong>Vehicle:</strong> {selectedReceiptBooking.ride.vehicleType} ({selectedReceiptBooking.ride.vehicleNumber || 'Campus Auto'})</p>
                <p><strong>Fare Split:</strong> ₹{selectedReceiptBooking.ride.pricePerSeat} × {selectedReceiptBooking.seatsBooked} seat = <strong>₹{selectedReceiptBooking.totalPrice}</strong></p>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-slate-100 flex justify-end">
              <button
                onClick={() => setSelectedReceiptBooking(null)}
                className="px-5 py-2.5 rounded-xl font-bold text-xs bg-slate-900 text-white hover:bg-slate-800 transition-colors"
              >
                Close Receipt
              </button>
            </div>
